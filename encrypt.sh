#!/bin/bash
# Script to decrypt a single password via Jasypt.
if [ $# -ne 2 ]; then
    echo "usage: ${0} [tst|acc|prd] [Password]"
    exit 1
fi
ENV="${1}"
INPUT="${2}"

BASE_DIR="$( cd "$( dirname "${BASH_SOURCE[0]}" )" >/dev/null 2>&1 && pwd )"

ENCRYPTION_JAR=${BASE_DIR}/lib/jasypt-encryption.jar
ENCRYPTION_CLASS=org.jasypt.intf.cli.JasyptPBEStringEncryptionCLI
ENCRYPTION_ALGORITHM=PBEWithHMACSHA512AndAES_256
IV_GENERATOR_CLASS=org.jasypt.iv.RandomIvGenerator

function jasypt_encrypt {
    FILE="${BASE_DIR}/lib/${1}"
    PASS="${2}"
    if [ ! -s "${FILE}" ] ; then
        echo "The script expects a decryption passwordin${FILE}"
        exit 1
    fi

    PASSWORD=$(cat ${FILE})

    echo "ENC("$(java -cp "${ENCRYPTION_JAR}" "${ENCRYPTION_CLASS}" \
                          input="${INPUT}" \
                          password="${PASSWORD}" \
                          algorithm="${ENCRYPTION_ALGORITHM}" \
                          ivGeneratorClassName="${IV_GENERATOR_CLASS}" \
                          verbose=false)")"
}

case $ENV in
    [tst]* ) jasypt_encrypt "jasypt_pwd_tst.txt" "${INPUT}" ;;
    [acc]* ) jasypt_encrypt "jasypt_pwd_acc.txt" "${INPUT}" ;;
    [prd]* ) jasypt_encrypt "jasypt_pwd_prd.txt" "${INPUT}" ;;
    * )     echo "Invalid environment provided. Should be [tst|acc|prd], supplied:${ENV}"
            exit 0
            ;;
esac